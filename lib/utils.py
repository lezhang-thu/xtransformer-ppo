...
def expand_tensor(tensor, size, dim=1):
    ...
    x_size = tensor.shape[dim - 1] * size
    tensor = tensor.view(
        list(tensor.shape[:dim - 1]) + [x_size] + list(tensor.shape[dim + 1:]))
    return tensor


def expand_numpy(x, num_samples=cfg.DATA_LOADER.SEQ_PER_IMG):
    if num_samples == 1:
        return x
    x = x.reshape((-1, 1))
    x = np.repeat(x, num_samples, axis=1)
    x = x.reshape((-1))
    return x
