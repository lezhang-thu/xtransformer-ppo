...
class Evaler(object):
    def __init__(self, eval_ids, gv_feat, att_feats, eval_annfile):
    ...
    def __call__(self, model, rname, beam_size=1):
        model.eval()

        results = []
        with torch.no_grad():
            for _, (indices, gv_feat, att_feats,
                    att_mask) in tqdm.tqdm(enumerate(self.eval_loader)):
                ...
                for sid, sent in enumerate(sents):
                    result = {
                        cfg.INFERENCE.ID_KEY: int(ids[sid]),
        ...
        json.dump(
            results,
            open(os.path.join(result_folder, 'result_' + rname + '.json'),
                 'w'))
