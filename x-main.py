...
class Trainer(object):
    ...
    def save_model(self, epoch):
        ...
        torch.save(self.model.state_dict(),
                   self.snapshot_path("caption_model", epoch + 1))

    def make_kwargs(self, indices, input_seq, target_seq, gv_feat, att_feats,
                    att_mask):
    ...
    def scheduled_sampling(self, epoch):
        if epoch > cfg.TRAIN.SCHEDULED_SAMPLING.START:
            frac = (epoch - cfg.TRAIN.SCHEDULED_SAMPLING.START
                    ) // cfg.TRAIN.SCHEDULED_SAMPLING.INC_EVERY
            ss_prob = min(cfg.TRAIN.SCHEDULED_SAMPLING.INC_PROB * frac,
                          cfg.TRAIN.SCHEDULED_SAMPLING.MAX_PROB)
    ...
    def display(self, iteration, data_time, batch_time, losses, loss_info):
        ...
        if self.distributed and dist.get_rank() > 0:
            return
        info_str = ' (DataTime/BatchTime: {:.3}/{:.3}) losses = {:.5}'.format(
            data_time.avg, batch_time.avg, losses.avg)
    ...
    def forward(self, kwargs):
        if self.rl_stage == False:
            ...
            rewards_sample = np.reshape(rewards_sample,
                                        (-1, cfg.DATA_LOADER.SEQ_PER_IMG))
            rewards_avg = np.mean(rewards_sample, axis=1)
            rewards = np.reshape(
                rewards_sample - np.expand_dims(rewards_avg, axis=1), (-1))
    ...
    def train(self):
        ...
        for epoch in range(cfg.SOLVER.MAX_EPOCH):
            ...
            for _, (indices, input_seq, target_seq, gv_feat, att_feats,
                ...
                #print("current_device: {}, att_feats.shape[0]: {}".format(
                #    torch.cuda.current_device(), att_feats.shape[0]))
                #exit(0)
