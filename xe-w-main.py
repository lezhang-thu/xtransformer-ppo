...
class Trainer(object):
    ...
    def _prefix_rewards(self, kwargs, seq_prefix):
        ...
        with torch.no_grad():
            seq_sample = self.model.module.extend_trajectory(
    ...
    def _sample_trajectory(self, kwargs):
        ...
        with torch.no_grad():
            seq_sample, log_prob_sample = self.model.module.decode(
                **kwargs)
            seq_sample_list = seq_sample.detach().cpu().numpy().tolist()
        ...
        kwargs['gx'], kwargs['encoder_out'], kwargs['p_att_feats'] , kwargs['att_mask']= \
                self.model.module.init_gx_encoder_out_p_att_feats_att_mask(**kwargs)
    ...
    def mb_train(self, kwargs):
        ...
        pg_loss = torch.max(pg_losses, pg_losses2).sum() / (cfg.TRAIN.BATCH_SIZE * 16)
