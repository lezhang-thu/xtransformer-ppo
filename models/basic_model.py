...
class BasicModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, **kwargs):
        ...
        beam_seq_table = [
            torch.LongTensor(cfg.MODEL.SEQ_LEN, bdash).zero_()
            for _ in range(group_size)
        ]
        beam_seq_logprobs_table = [
            torch.FloatTensor(cfg.MODEL.SEQ_LEN, bdash).zero_()
            for _ in range(group_size)
        ]
                ...
                if t >= divm and t <= cfg.MODEL.SEQ_LEN + divm - 1:
                    ...
                    for vix in range(bdash):
                        if beam_seq_table[divm][
                                t - divm,
                                vix] == 0 or t == cfg.MODEL.SEQ_LEN + divm - 1:
                            final_beam = {
                                'seq':
                    ...
                    logprobs_table[divm], state_table[
                        divm] = self.get_logprobs_state(**kwargs)
