...
class XTransformer(BasicModel):
    ...
    def get_logprobs_state(self, **kwargs):
        ...
        seq_mask = subsequent_mask(ys.size(1)).to(encoder_out.device).type(
            torch.cuda.FloatTensor)[:, -1, :].unsqueeze(1)
        decoder_out = self.decoder(gx, ys[:, -1].unsqueeze(-1), encoder_out,
                                   att_mask, seq_mask, p_att_feats,
                                   True).squeeze(1)

        logprobs = F.log_softmax(decoder_out, dim=-1)
        return logprobs, [ys.unsqueeze(0)]

    def _expand_state(self, batch_size, beam_size, cur_beam_size,
                      selected_beam):
        def fn(s):
            ...
            s = torch.gather(
                s.view(*([batch_size, cur_beam_size] + shape[1:])), 1,
    ...
    def decode_beam(self, **kwargs):
        ...
        #seq_mask = torch.ones((batch_size, 1, 1)).cuda()
    ...
    def extend_trajectory(self, **kwargs):
        batch_size = kwargs['encoder_out'].size(0)
        self.decoder.init_buffer(batch_size)

        state = None
        sents = torch.zeros((batch_size, cfg.MODEL.SEQ_LEN),
                            dtype=torch.long).cuda()
        max_gen_len = kwargs[cfg.PARAM.MAX_GEN_LEN]
        gen_result = kwargs[cfg.PARAM.GEN_RESULT].view(batch_size, max_gen_len)

        wt = torch.zeros(batch_size, dtype=torch.long).cuda()
        unfinished = wt.eq(wt)
        kwargs[cfg.PARAM.ATT_FEATS] = kwargs['encoder_out']
        kwargs[cfg.PARAM.GLOBAL_FEAT] = kwargs['gx']
        kwargs[cfg.PARAM.P_ATT_FEATS] = kwargs['p_att_feats']
        kwargs[cfg.PARAM.ATT_FEATS_MASK] = kwargs['att_mask']
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            if t < max_gen_len:
                ...
                wt = torch.multinomial(torch.exp(logprobs_t), 1)
...
class Encoder(nn.Module):
    def __init__(self, embed_dim, dropout, att_type, att_heads, att_mid_dim,
                 att_mid_drop, bifeat_emb_act, bifeat_emb_drop, ff_dropout,
                 layer_num):
        super(Encoder, self).__init__()
        self.att_heads = att_heads
        self.layers = nn.ModuleList([])
        for i in range(layer_num):
            sublayer = EncoderLayer(embed_dim=embed_dim,
                                    dropout=dropout,
                                    att_type=att_type,
                                    att_heads=att_heads,
    ...
    def forward(self, x, mask):
        gx = (torch.sum(x * mask.unsqueeze(-1), 1) /
              torch.sum(mask.unsqueeze(-1), 1))
...
class EncoderLayer(nn.Module):
    def __init__(self, embed_dim, dropout, att_type, att_heads, att_mid_dim,
        ...
        self.bifeat_emb = nn.Sequential(nn.Linear(2 * embed_dim, embed_dim),
    ...
    def forward(self, gx, x, mask):
        gx = self.encoder_attn(query=gx, key=x, mask=mask, value1=gx, value2=x)
        gx = self.dropout(gx)
...
class Decoder(nn.Module):
    ...
    def forward(self,
        ...
        gx = self.wbil1(gx)
        if self.x is None:
            x_gx = (torch.sum(x.unsqueeze(1) * seq_mask.unsqueeze(-1), -2) /
                    torch.sum(seq_mask, -1).unsqueeze(-1))
        else:
            self.x = self.x + x
            x_gx = self.x / seq_len
