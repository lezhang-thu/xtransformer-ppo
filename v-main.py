...
class Trainer(object):
    ...
    def _expand_kwargs(kwargs):
        ...
        indices = utils.expand_numpy(indices)
        gv_feat, att_feats, att_mask = [
            utils.expand_tensor(_, cfg.DATA_LOADER.SEQ_PER_IMG)
    ...
    def _sample_trajectory(self, kwargs):
        kwargs = Trainer._expand_kwargs(kwargs)
        ...
        with torch.no_grad():
            ...
            log_prob_sample, seq_sample = [
                _.detach().cpu().numpy().reshape(
                    (cfg.TRAIN.BATCH_SIZE, cfg.DATA_LOADER.SEQ_PER_IMG,
                     *_.shape[1:])) for _ in [log_prob_sample, seq_sample]
            ]
